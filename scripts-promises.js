...
function main(){
    console.log("Promises Js File being used....")
    let dataPromise;
    let cachedBooks = localStorage.getItem("booksData");

    if(cachedBooks){
        console.log("Loaded from Local Storage");
        dataPromise = Promise.resolve(JSON.parse(cachedBooks));
    }
    else{
        ...
        .then(books => {let processedBooks = books.map((book,index) => ({sno:index+1,...book,lang:"English"}));
        localStorage.setItem("booksData", JSON.stringify(processedBooks));
        return processedBooks;})
        .catch(error => {
            console.error("Error loading book data:", error);
            alert("Failed to load books data. Please refresh or check your connection.");
            return [];
        });
    }
    // .then(books => mainBooks = books.map((book,index) => ({sno:index+1,...book,lang:"English"})))
    dataPromise
    .then(books => {
    ...
    .then(() => pagination(allBooks)) //To update Page Numbers
    // .then(() => console.log(allBooks.length))
    // .then(() => console.log(allBooks[0])) //Testing
    //.then(() => display(allBooks,0,10))
    // .then(() => renderBooks(allBooks,'table-all-books-body'))
    // .then(() => {for (let val of allBooks){console.log(val);};}
    // .then(() => renderBooks(allBooks,'table-similar-books-body'))
    //console.log("Loaded the DOM - Test by Shivam - Inside Func")
